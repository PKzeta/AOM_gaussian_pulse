...
class AWGGaussianFlatTopPulse:
    ...
    def update_pulse(
        ...
        carrier_two = np.sin(2 * np.pi * frequency_hz * t_us * 1e-6 + phase)

        full_scale = 32767
        ch0 = np.clip(envelope * carrier * full_scale, -full_scale, full_scale).astype(np.int16)
        ch1 = np.clip(carrier_two * full_scale, -full_scale, full_scale).astype(np.int16)
        ...
        spcm_dwSetParam_i32(self.hCard, SPC_AMP0, int(amplitude_mV))
        spcm_dwSetParam_i32(self.hCard, SPC_AMP1, int(amplitude_mV))
...
if __name__ == "__main__":
    awg = AWGGaussianFlatTopPulse(device="/dev/spcm0", samplerate=1_250_000_000)
    ...
    try:
        while True:
            ...
            try:
                ...
                phase = float(input("phase (radians): "))
                amplitude_mV = float(input("peak amplitude (mV) (<100 mV): "))
                if amplitude_mV > 10000:
